...
var resizePizzas = function(size) {
  ...
  // Improved changePizzaSizes by avoiding redundancy
  function changePizzaSizes(size) {
    ...
    // Select all Pizza elements
    // avoid redundancy by storing randomPizzas  	
  	var randomPizzas = document.querySelectorAll(".randomPizzaContainer");
  	// loop through Pizzas and change their width
    for (var i = 0; i < randomPizzas.length; i++) {
    	randomPizzas[i].style.width = newWidth + "%";
      // the value of newWidth depends on the slider
    }
  }
...
function updatePositions() {
  ...
  // select items by class instead of querry Selector => faster
  var items = document.getElementsByClassName("mover");
  // move document.body.scrolltop out of loop because it is costly
  var scroll_documents = document.body.scrollTop / 1250;
  // Use a counter instead of modulo operator
  var counter = 0;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(scroll_documents + counter);
    if (counter > 4){ // reset counter if it is bigger than 4
    	counter = 0;
    } else {
    	counter = counter + 1;
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < NUMBERPIZZAS; i++) {
  	// reduce the number of pizza instances created, 200 is too much
